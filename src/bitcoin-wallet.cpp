...
static bool WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s vertcoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";

        if (args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                        "vertcoin-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
...
int main(int argc, char* argv[])
{
    ...
    if (!command) {
        tfm::format(std::cerr, "No method provided. Run `vertcoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
